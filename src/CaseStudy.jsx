...
const slideLeft = { hidden: { opacity: 0, x: -20 }, visible: { opacity: 1, x: 0, transition: { duration: 0.5, ease: [0.22, 1, 0.36, 1] } } }
...
function ScreenBox({ label, aspect = '16/9', bg = '#1a2a22' }) {
  return (
    ...
    <AnimatePresence>
      {visible && (
        <motion.nav initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}
          ...
          className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-white/90 backdrop-blur-md border border-black/[0.07] rounded-full px-2 py-1.5 flex items-center gap-0.5 shadow-[0_4px_20px_rgba(0,0,0,0.1)]">
          {NAV.map(item => (
    ...
    <section id="hero" ref={ref} className="bg-white overflow-hidden relative">
      ...
      <div className="relative max-w-5xl mx-auto px-6 pt-28 pb-0 text-center">
        ...
        <motion.div style={{ y: imgY }} initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, delay: 0.5 }} className="relative mx-auto max-w-4xl">
...
const TOKENS = [
  ...
  { name: 'Base Dark', hex: '#0D1F18', role: 'Page bg (dark mode)' },
    ...
    <section id="design-system" style={{ background: '#FDF8EC' }} className="py-24 px-6">
      <div className="max-w-5xl mx-auto">
        <RevealGroup>
          ...
          <motion.p variants={fadeUp} className="text-ink-3 font-body mb-16 max-w-lg text-[0.95rem] leading-relaxed">
            A complete glassmorphic design language — color tokens, type scale, spacing system, and 60+ components.
          </motion.p>
        </RevealGroup>
              ...
              <div className="grid grid-cols-3 gap-3">
                {TOKENS.map(t => (
                  <div key={t.name} className="group">
                    <div className="h-14 rounded-xl mb-2 transition-transform duration-200 group-hover:-translate-y-0.5 group-hover:shadow-md" style={{ background: t.hex }} />
              ...
              {[
                ...
                <div key={state.label} className="rounded-xl overflow-hidden" style={{ background: state.bg, padding: 16 }}>
                  <div className="rounded-lg p-4" style={{ background: state.cardBg, border: `1px solid ${state.border}`, backdropFilter: 'blur(12px)' }}>
                    ...
                    <div className="h-1.5 rounded-full bg-white/[0.06] w-1/2" />
                  </div>
...
const LEARNINGS = [
  ...
  { icon: '♿', title: 'Accessibility unlocks everyone', body: 'WCAG 2.2 AA contrast and touch targets made the UI better for all users. It needs to be a launch gate, not a post-launch checklist.', color: '#008062' },
  { icon: '🤖', title: 'AI needs conservative UI', body: 'Users didn\'t trust AI suggestions without a rationale. Pairing every output with its reasoning doubled confidence scores in usability testing.', color: '#ED2525' },
]
